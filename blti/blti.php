...
function is_basic_lti_request() {
   ...
   $resource_link_id = isset($_REQUEST["resource_link_id"]) && $_REQUEST["resource_link_id"];
...
class TrivialOAuthDataStore extends OAuthDataStore {
	...
	function lookup_nonce($consumer, $token, $nonce, $timestamp) {
		// Should add some clever logic to keep nonces from being reused -
		// for no we are really trusting that the timestamp will save us
		return NULL;
	}
...
class BLTI {
	...
	function __construct($parm=false, $usesession=true, $doredirect=true) {
		...
		if ( is_string($parm) ) {
			...
			$this->message = "Constructor requires a secret.";
			return;
		}
